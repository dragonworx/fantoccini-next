...
describe('Keyframes and AnimatableProperty', () => {
  ...
  describe('Non-numeric Values', () => {
    it('should handle string values with step interpolation', () => {
      ...
      stringProperty.addKeyframe({ time: 1, value: 'first', interpolation: 'Linear' });
      stringProperty.addKeyframe({ time: 3, value: 'second', interpolation: 'Linear' });
  ...
  describe('Bezier Interpolation', () => {
    ...
    it('should fall back to linear for non-numeric values', () => {
      ...
      stringProperty.addKeyframe({ time: 0, value: 'start', interpolation: 'Linear' });
      stringProperty.addKeyframe({ time: 1, value: 'end', interpolation: 'Bezier' });
    ...
    it('should work with helper method for creating easing keyframes', () => {
      property.addKeyframe({ time: 0, value: 0, interpolation: 'Linear' });
      property.addKeyframe(AnimatableProperty.createEasingKeyframe(2, 100, 'ease-in'));
    ...
    it('should support all standard easing curves', () => {
      ...
      easings.forEach(easing => {
        ...
        testProperty.addKeyframe({ time: 0, value: 0, interpolation: 'Linear' });
        testProperty.addKeyframe(AnimatableProperty.createEasingKeyframe(1, 100, easing));
  ...
  describe('Catmull-Rom Spline Interpolation', () => {
    ...
    it('should create smooth curves through multiple points', () => {
      ...
      property.addKeyframe({ time: 0, value: 0, interpolation: 'Linear' });
      property.addKeyframe({ time: 1, value: 100, interpolation: 'CatmullRom' });
      property.addKeyframe({ time: 2, value: 0, interpolation: 'CatmullRom' });
      property.addKeyframe({ time: 3, value: -100, interpolation: 'CatmullRom' });
    ...
    it('should fall back to linear for non-numeric values', () => {
      ...
      colorProperty.addKeyframe({ time: 0, value: 'red', interpolation: 'Linear' });
      colorProperty.addKeyframe({ time: 1, value: 'blue', interpolation: 'CatmullRom' });
  ...
  describe('Mixed Interpolation Types', () => {
    ...
    it('should handle different interpolation types in sequence', () => {
      ...
      property.addKeyframe({ time: 2, value: 100, interpolation: 'Linear' });
      property.addKeyframe(AnimatableProperty.createEasingKeyframe(3, 0, 'ease-out'));
      property.addKeyframe(AnimatableProperty.createCatmullRomKeyframe(4, 75));

      // Test each segment
      expect(property.resolveValue(0.5)).toBe(25); // Linear
      expect(property.resolveValue(1.5)).toBe(50); // Step
  ...
  describe('Performance and Precision', () => {
    it('should handle Bezier curves with high precision', () => {
      ...
      const samples = [0, 0.25, 0.5, 0.75, 1].map(t => property.resolveValue(t));
    ...
    it('should handle complex Catmull-Rom splines efficiently', () => {
      ...
      for (let i = 0; i <= 10; i++) {
        ...
        property.addKeyframe(AnimatableProperty.createCatmullRomKeyframe(i, value));
      }
    ...
    it('should solve Bezier curves with Newton-Raphson accurately', () => {
      ...
      // With fast start control point, early value should be high
      expect(earlyValue).toBeGreaterThan(50);
      // Mid should be somewhere in the middle
      expect(midValue).toBeGreaterThan(20);
      expect(midValue).toBeLessThan(80);
      // Late value should be low due to slow end
      expect(lateValue).toBeLessThan(50);
    });
...
describe('DOMTimelineObject', () => {
  ...
  it('should apply transform styles to DOM element', () => {
    ...
    rotationProperty.addKeyframe({ time: 0, value: 0, interpolation: 'Linear' });
    rotationProperty.addKeyframe({ time: 1, value: 45, interpolation: 'Linear' });

    domObject.update(0.5);

    expect(element.style.transform).toBe('translate(50px, 25px) rotate(22.5deg) scale(1, 1)');
  });
...
describe('Timeline', () => {
  ...
  describe('Hierarchical Time Updates', () => {
    ...
    it('should update through multiple hierarchy levels', () => {
      const grandchildTimeline = new Timeline({ startTime: 0.5, timeScale: 0.5 });
      ...
      expect(grandchildObject.getCurrentValue('nested')).toBe(350); // 1.75 * 200
    });
  });
...
describe('Integration Tests', () => {
  describe('Complex Animation Scenario', () => {
    it('should handle a complex nested animation with multiple objects', () => {
      ...
      const characterTimeline = new Timeline({ startTime: 1, duration: 5, timeScale: 1.5 });
      const backgroundTimeline = new Timeline({ startTime: 0, duration: 8, timeScale: 0.8 });
      ...
      // Test character bobbing (repeating pattern)
      const bobValue = character.getCurrentValue('y');
      expect(bobValue).toBeCloseTo(92.5, 1); // Interpolated value based on cycle
    });
  });
